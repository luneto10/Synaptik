...
func buildTableDDLForDialect(dialect ddlspec.Dialect, t diagram.DbTable, colIndex map[diagram.ColumnID]ddlspec.ColumnInfo) ([]string, error) {
	pkCols := getPrimaryKeyColumns(t)
	compositePK := t.IsJunction() && len(pkCols) > 1

	colDefs := make([]string, 0, len(t.Columns())+1)
	for _, c := range t.Columns() {
		var reference *ddlspec.ColumnInfo
		if c.References() != nil {
		...
		columnDef, err := dialect.BuildColumnDefinition(c, ddlspec.ColumnContext{
